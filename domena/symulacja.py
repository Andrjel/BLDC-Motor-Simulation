...
class Symulacja:
    ...
    def aktualizacja_symulacji(self, wejscie):
        ...
        if wejscie is None:
            return
        self.dane["czas"].dodanie_do_kolejki(self.dane['czas'].ostatnia_wartosc() +
                                             self.__czas_probkowania)
        self.dane['wejscie'].dodanie_do_kolejki(wejscie)
        _, y, _ = signal.lsim(self.__silnik,
                              U=self.dane['wejscie'].aktualna_kolejka(),
